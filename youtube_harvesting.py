...
def connect_api_key():
    api_key="AIzaSyBeC6aoFyVXDN7pQIa4SaXBIoFDaWPKZto"
        ...
        for video_id in Video_ids:
            ...
            for i in response_comment['items']:
                data_3 = dict(comment_id=i['snippet']['topLevelComment']['id'],
                              ...
                              comment_publishedat=i['snippet']['topLevelComment']['snippet']['publishedAt'].replace("T",
                                                                                                                    " ").replace(
                                  "Z", " "))
...
with slt.sidebar:
    web=option_menu(
        menu_title="YOUTUBE-DATA-HARVESTING-AND-DATA-WAREHOUSING",
...
if web == "HOME":
    ...
    slt.subheader(":blue[Domain:]  Social Media")
    ...
    slt.subheader(":blue[Developed-by:]  Sathish Tamildhasan")
...
cursor = conn.cursor()
query = "create database if not exists youtube"
...
if web == "MIGRATE TO SQL":
    ...
    if slt.button("migrate to sql"):
        conn = mysql.connector.connect(host="localhost", user="root", password="12345678", database="youtube")
...
if web == "DATA ANALYSIS AND VISUALIZATION":
    ...
    option_menu = slt.selectbox("Select options",
...
if option_menu == "1.What are the names of all the videos and their corresponding channels?":
    if slt.button("SUBMIT"):
            conn = mysql.connector.connect(host="localhost", user="root", password="12345678", database="youtube")
            my_cursor = conn.cursor()
            ...
            que_1 = pd.DataFrame(out, columns=["video_name", "channel_name"])
            slt.success("ANSWER")
            slt.write(que_1)

# Query to excute 2nd Question
if option_menu == "2.Which channels have the most number of videos, and how many videos do they have?":
    if slt.button("SUBMIT"):
            conn = mysql.connector.connect(host="localhost", user="root", password="12345678", database="youtube")
            my_cursor = conn.cursor()
...
if option_menu == "3.What are the top 10 most viewed videos and their respective channels?":
    if slt.button("SUBMIT"):
            conn = mysql.connector.connect(host="localhost", user="root", password="12345678", database="youtube")
            my_cursor = conn.cursor()
...
if option_menu == "4.How many comments were made on each video, and what are their corresponding video names?":
    if slt.button("SUBMIT"):
            conn = mysql.connector.connect(host="localhost", user="root", password="12345678", database="youtube")
            my_cursor = conn.cursor()
...
if option_menu == "5.Which videos have the highest number of likes, and what are their corresponding channel names?":
    if slt.button("SUBMIT"):
            conn = mysql.connector.connect(host="localhost", user="root", password="12345678", database="youtube")
            my_cursor = conn.cursor()
...
if option_menu == "6.What is the total number of likes and dislikes for each video, and what are  their corresponding video names?":
    if slt.button("SUBMIT"):
            conn = mysql.connector.connect(host="localhost", user="root", password="12345678", database="youtube")
            my_cursor = conn.cursor()
...
if option_menu == "7.What is the total number of views for each channel, and what are their corresponding channel names?":
    if slt.button("SUBMIT"):
            conn = mysql.connector.connect(host="localhost", user="root", password="12345678", database="youtube")
            my_cursor = conn.cursor()
...
if option_menu == "8.What are the names of all the channels that have published videos in the year 2022?":
    if slt.button("SUBMIT"):
            conn = mysql.connector.connect(host="localhost", user="root", password="12345678", database="youtube")
            my_cursor = conn.cursor()
...
if option_menu == "9.What is the average duration of all videos in each channel, and what are their corresponding channel names?":
    if slt.button("SUBMIT"):
            conn = mysql.connector.connect(host="localhost", user="root", password="12345678", database="youtube")
            my_cursor = conn.cursor()
...
if option_menu == "10.Which videos have the highest number of comments, and what are their corresponding channel names?":
    if slt.button("SUBMIT"):
            conn = mysql.connector.connect(host="localhost", user="root", password="12345678", database="youtube")
            my_cursor = conn.cursor()
            ...
            # Setting up the option "Data Visualization" in streamlit page
            fix = px.bar(que_10, x="channel_name", y="comment_count", title="HIGHEST COMMENTS COUNT",
                         color="channel_name")
            slt.plotly_chart(fix)
